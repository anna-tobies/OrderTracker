...
const DWELL_MS = 4000; // 4 seconds — adjust as needed

// ── Moving Average Filter ─────────────────────────────────────────────────
// Keeps the last N positions per tag and uses the average for zone detection
const MOVING_AVG_WINDOW = 3; // number of positions to average — increase for more smoothing
const tagPositionHistory = {};
// Format: { [tagId]: [ {x, y}, {x, y}, ... ] }

function getSmoothedPosition(tagId, rawX, rawY) {
  if (!tagPositionHistory[tagId]) tagPositionHistory[tagId] = [];
  const history = tagPositionHistory[tagId];
...
async function handlePozyxMessage(data) {
  ...
  const raw = { x: coords.x, y: coords.y };
  const { x, y } = getSmoothedPosition(tagId, raw.x, raw.y);
      ...
      if (activeSessions[key]) {
        ...
        if (!dwellTimers[`exit:${key}`]) {
          dwellTimers[`exit:${key}`] = {
            ...
            }, 4000), // 4s exit debounce time to filter positional jitter at zone boundary
            timestamp: ts,
          };
        }
      }
